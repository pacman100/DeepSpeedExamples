...
from utils.model.model_utils import create_hf_model, create_critic_model, create_actor_critic_model
...
class PeftDeepSpeedRLHFEngine:
    def __init__(self, actor_model_name_or_path, critic_model_name_or_path, tokenizer, args, num_total_iters):
        ...
        optimizer_grouped_parameters = [
            {
                "params": [p for n, p in model.named_parameters() if (("actor" in n) and p.requires_grad)],
                "weight_decay": self.args.actor_weight_decay,
                "lr": self.args.actor_learning_rate,
            },
            {
                "params": [p for n, p in model.named_parameters() if (("critic" in n) and p.requires_grad)],
                "weight_decay": self.args.actor_weight_decay,
                "lr": self.args.actor_learning_rate,
            },
        ]

        optim = AdamOptimizer(optimizer_grouped_parameters, lr=self.args.actor_learning_rate, betas=(0.9, 0.95))
        ...
        # DeepSpeed Engine
        # TODO: move enable_hybrid_engine and pin_parameters to ds_config
        engine, *_ = deepspeed.initialize(model=model, optimizer=optim, lr_scheduler=lr_scheduler, config=ds_config)
        self.engine = engine

        log_init("Actor-Critic", stime=stime)
