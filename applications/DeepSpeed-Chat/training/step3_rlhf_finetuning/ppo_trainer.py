...
class PeftDeepSpeedPPOTrainer:
    def __init__(self, rlhf_engine, args):
        self.rlhf_engine = rlhf_engine
        self.actor_critic_model = rlhf_engine.engine.module
        self.tokenizer = self.rlhf_engine.tokenizer
        self.args = args
        self.max_answer_seq_len = args.max_answer_seq_len
        self.end_of_conversation_token_id = self.tokenizer(args.end_of_conversation_token)["input_ids"][-1]
    ...
    def _generate_sequence(self, prompts):
        ...
        with torch.no_grad():
            seq = self.actor_critic_model.generate(prompts, max_length=max_min_length, min_length=max_min_length)
    ...
    def generate_experience(self, prompts):
        self.actor_critic_model.set_adapter("actor")
        self.eval()
        if self.args.gradient_checkpointing:
            self.actor_critic_model.gradient_checkpointing_disable()
            self.actor_critic_model.config.use_cache = True
        seq = self._generate_sequence(prompts)
        ...
        self.train()
        if self.args.gradient_checkpointing:
            self.actor_critic_model.gradient_checkpointing_enable()
            self.actor_critic_model.config.use_cache = False

        with torch.no_grad():
            self.actor_critic_model.set_adapter("actor")
            output = self.rlhf_engine.engine(seq, attention_mask=attention_mask)
    ...
    def train_rlhf(self, inputs):
        ...
        self.actor_critic_model.set_adapter("actor")
        batch = {"input_ids": seq, "attention_mask": attention_mask}
        actor_prob = self.rlhf_engine.engine(**batch, use_cache=False).logits
        actor_log_prob = gather_log_probs(actor_prob[:, :-1, :], inputs["input_ids"][:, 1:])
        actor_loss = self.actor_loss_fn(
            actor_log_prob[:, start:], log_probs[:, start:], advantages, action_mask[:, start:]
        )
        self.rlhf_engine.engine.backward(actor_loss)
    ...
    def _validate_training_mode(self):
        assert self.actor_critic_model.training

    def _validate_evaluation_mode(self):
        assert not self.actor_critic_model.training
    ...
    def dump_model_norms(self, tag):
        model_norm = get_model_norm(self.actor_critic_model)
        print_all_ranks(f"{tag} global_actor_critic_model_norm", model_norm, self.args.local_rank)
