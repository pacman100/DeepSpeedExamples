...
from ppo_trainer import DeepSpeedPPOTrainer, DeepSpeedPPOTrainerUnsupervised, PeftDeepSpeedPPOTrainer
from rlhf_engine import DeepSpeedRLHFEngine, PeftDeepSpeedRLHFEngine
...
def parse_args():
    ...
    parser.add_argument(
        "--unsup_coef", type=float, default=27.8, help="""gamma in Equation 2 from InstructGPT paper"""
    )
    parser.add_argument(
        "--model_name_or_path",
        type=str,
    ...
    parser.add_argument("--actor_weight_decay", type=float, default=0.1, help="Weight decay to use.")
    parser.add_argument("--critic_weight_decay", type=float, default=0.1, help="Weight decay to use.")
    ...
    parser.add_argument(
        "--zero_stage", type=int, default=0, help="ZeRO optimization stage for Actor Critic model (and clones)."
    )
    parser.add_argument(
        "--gradient_checkpointing",
        action="store_true",
        help="Enable HF gradient checkpointing for Actor Critic model.",
    )
...
def main():
    ...
    rlhf_engine = PeftDeepSpeedRLHFEngine(
    ...
    ppo_trainer = PeftDeepSpeedPPOTrainer
            ...
            if exp_dataset is not None:
                ...
                for ppo_ep in range(args.ppo_epochs):
                    for i, (exp_data, unsup_data) in enumerate(zip(exp_dataset, unsup_dataset)):
                        ...
                        inner_iter += 1
                        if args.enable_ema:
                            peft_moving_average(rlhf_engine.engine.module, zero_stage=args.zero_stage)
    ...
    if args.output_dir is not None:
        ...
        model_to_save = rlhf_engine.engine.module if hasattr(rlhf_engine.engine, "module") else rlhf_engine.engine
        if args.zero_stage == 3:
            if rlhf_engine.engine.zero_gather_16bit_weights_on_model_save():
                state_dict = rlhf_engine.engine._zero3_consolidated_16bit_state_dict()
            else:
                raise ValueError(
